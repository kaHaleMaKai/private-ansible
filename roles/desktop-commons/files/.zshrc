...
venv() {
	if [[ "$1" = "" ]]; then
		echo "no venv dir given" >&2
		return 1
	fi
        NEW_VENV="/home/lars/python3-venvs/${1}"
	CMD="${NEW_VENV}/bin/activate"
	CUR_VENV="${HOME}/cur_venv"
	rm -f "$CUR_VENV" 2>/dev/null &&
		ln -s -T "${NEW_VENV}" "$CUR_VENV"
	source "$CMD"
}
